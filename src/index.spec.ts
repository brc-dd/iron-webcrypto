...
describe('Iron', () => {
  ...
  it('fails to turns object into a ticket (password buffer too short)', async () => {
    ...
    await expect(Iron.seal(crypto, obj, key, Iron.defaults)).rejects.toThrow(
  ...
  it('fails to turn object into a ticket (failed to stringify object)', async () => {
    ...
    cyclic[0] = cyclic
    const key = Iron.randomBits(crypto, 128)
    await expect(Iron.seal(crypto, cyclic, key, Iron.defaults)).rejects.toThrow(
  ...
  it('fails to parse a sealed object when password not found', async () => {
    const sealed = await Iron.seal(crypto, obj, { id: '1', secret: password }, Iron.defaults)
    await expect(Iron.unseal(crypto, sealed, { '2': password }, Iron.defaults)).rejects.toThrow(
  ...
  describe('generateKey()', () => {
    it('returns an error when password is missing', async () => {
      // @ts-expect-error
      await expect(Iron.generateKey(crypto, null, null)).rejects.toThrow('Empty password')
    })

    it('returns an error when password is too short', async () => {
      await expect(Iron.generateKey(crypto, 'password', Iron.defaults.encryption)).rejects.toThrow(
    ...
    it('returns an error when options are missing', async () => {
      // @ts-expect-error
      await expect(Iron.generateKey(crypto, password, null)).rejects.toThrow('Bad options')
      // @ts-expect-error
      await expect(Iron.generateKey(crypto, password, 'abc')).rejects.toThrow('Bad options')
    })
  ...
  describe('encrypt()', () => {
    it('returns an error when password is missing', async () => {
      // @ts-expect-error
      await expect(Iron.encrypt(crypto, null, null, 'data')).rejects.toThrow('Empty password')
    })
  })
  ...
  describe('decrypt', () => {
    it('returns an error when password is missing', async () => {
      // @ts-expect-error
      await expect(Iron.decrypt(crypto, null, null, 'data')).rejects.toThrow('Empty password')
    })
  })
  ...
  describe('hmacWithPassword()', () => {
    it('returns an error when password is missing', async () => {
      // @ts-expect-error
      await expect(Iron.hmacWithPassword(crypto, null, null, 'data')).rejects.toThrow(
        'Empty password'
      )
    })
  ...
  describe('seal()', () => {
    it('returns an error when password is missing', async () => {
      // @ts-expect-error
      await expect(Iron.seal(crypto, 'data', null, Iron.defaults)).rejects.toThrow('Empty password')
    })
    ...
    it('returns an error when integrity options are missing', async () => {
      ...
      await expect(Iron.seal(crypto, 'data', password, options)).rejects.toThrow(
  ...
  describe('unseal()', () => {
    ...
    it('returns an error when password is missing', async () => {
      ...
      // @ts-expect-error
      await expect(Iron.unseal(crypto, ticket, null, Iron.defaults)).rejects.toThrow(
        'Empty password'
      )
    })
