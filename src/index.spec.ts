...
describe('Iron', () => {
  ...
  test('turns object into a ticket than parses the ticket successfully', async () => {
  ...
  test('unseal and sealed object with expiration', async () => {
  ...
  test('unseal and sealed object with expiration and time offset', async () => {
    const options = Iron.clone(Iron.defaults);
    options.ttl = 200;
  ...
  test('turns object into a ticket than parses the ticket successfully (password buffer)', async () => {
  ...
  test('turns object into a ticket than parses the ticket successfully (password buffer in object)', async () => {
    const key = Iron.randomBits(_crypto, 256);
    const sealed = await Iron.seal(_crypto, obj, key, Iron.defaults);
  ...
  test('fails to turns object into a ticket (password buffer too short)', async () => {
    const key = Iron.randomBits(_crypto, 128);
    await expect(Iron.seal(_crypto, obj, key, Iron.defaults)).rejects.toThrow(
      'Key buffer (password) too small',
    );
  });

  test('fails to turn object into a ticket (failed to stringify object)', async () => {
    const cyclic: Array<unknown> = [];
    cyclic[0] = cyclic;
    const key = Iron.randomBits(_crypto, 128);
    await expect(Iron.seal(_crypto, cyclic, key, Iron.defaults)).rejects.toThrow(
      'Converting circular structure to JSON',
    );
  });

  test('turns object into a ticket than parses the ticket successfully (password object)', async () => {
  ...
  test('handles separate password buffers (password object)', async () => {
    const key = {
      id: '1',
  ...
  test('handles a common password buffer (password object)', async () => {
  ...
  test('fails to parse a sealed object when password not found', async () => {
    const sealed = await Iron.seal(_crypto, obj, { id: '1', secret: password }, Iron.defaults);
    await expect(Iron.unseal(_crypto, sealed, { '2': password }, Iron.defaults)).rejects.toThrow(
      'Cannot find password: 1',
    );
  });

  describe('generateKey()', () => {
    test('returns an error when password is missing', async () => {
      // @ts-expect-error
      await expect(Iron.generateKey(_crypto, null, null)).rejects.toThrow('Empty password');
    });

    test('returns an error when password is too short', async () => {
      await expect(Iron.generateKey(_crypto, 'password', Iron.defaults.encryption)).rejects.toThrow(
        'Password string too short (min 32 characters required)',
      );
    });

    test('returns an error when options are missing', async () => {
      // @ts-expect-error
      await expect(Iron.generateKey(_crypto, password, null)).rejects.toThrow('Bad options');
      // @ts-expect-error
      await expect(Iron.generateKey(_crypto, password, 'abc')).rejects.toThrow('Bad options');
    });

    test('returns an error when an unknown algorithm is specified', async () => {
      await expect(
        // @ts-expect-error
    ...
    test('returns an error when no salt and no salt bits are provided', async () => {
      const options = { algorithm: 'sha256' as const, iterations: 2, minPasswordlength: 32 };
      await expect(Iron.generateKey(_crypto, password, options)).rejects.toThrow(
        'Missing salt and saltBits options',
      );
    });

    test('returns an error when invalid salt bits are provided', async () => {
      const options = {
  ...
  describe('encrypt()', () => {
    test('returns an error when password is missing', async () => {
      // @ts-expect-error
      await expect(Iron.encrypt(_crypto, null, null, 'data')).rejects.toThrow('Empty password');
    });
  });
  ...
  describe('decrypt', () => {
    test('returns an error when password is missing', async () => {
      // @ts-expect-error
      await expect(Iron.decrypt(_crypto, null, null, 'data')).rejects.toThrow('Empty password');
    });
  });
  ...
  describe('hmacWithPassword()', () => {
    test('returns an error when password is missing', async () => {
      // @ts-expect-error
      await expect(Iron.hmacWithPassword(_crypto, null, null, 'data')).rejects.toThrow(
        'Empty password',
      );
    });

    test('produces the same mac when used with buffer password', async () => {
  ...
  describe('seal()', () => {
    test('returns an error when password is missing', async () => {
      // @ts-expect-error
      await expect(Iron.seal(_crypto, 'data', null, Iron.defaults)).rejects.toThrow(
        'Empty password',
      );
    });

    test('returns an error when integrity options are missing', async () => {
      const options = Iron.clone(Iron.defaults);
      // @ts-expect-error
      options.integrity = {};

      await expect(Iron.seal(_crypto, 'data', password, options)).rejects.toThrow(
        'Unknown algorithm: undefined',
      );
    });

    test('returns an error when password.id is invalid', async () => {
      await expect(
        Iron.seal(_crypto, 'data', { id: 'asd$', secret: 'asd' }, Iron.defaults),
      ).rejects.toThrow('Invalid password id');
    });
  });

  describe('unseal()', () => {
    test('unseals a ticket', async () => {
      const ticket =
    ...
    test('returns an error when number of sealed components is wrong', async () => {
      const ticket =
      ...
      await expect(Iron.unseal(_crypto, ticket, password, Iron.defaults)).rejects.toThrow(
    ...
    test('returns an error when password is missing', async () => {
      ...
      // @ts-expect-error
      await expect(Iron.unseal(_crypto, ticket, null, Iron.defaults)).rejects.toThrow(
        'Empty password',
      );
    });

    test('returns an error when mac prefix is wrong', async () => {
      const ticket =
    ...
    test('returns an error when integrity check fails', async () => {
      const ticket =
    ...
    test('returns an error when decryption fails', async () => {
      const macBaseString =
    ...
    test('returns an error when iv base64 decoding fails', async () => {
      const macBaseString =
        'Fe26.2**a6dc6339e5ea5dfe7a135631cf3b7dcf47ea38246369d45767c928ea81781694*D3DLEoi-Hn3c972TPpZXqw??*mCBhmhHhRKk9KtBjwu3h-1lx1MHKkgloQPKRkQZxpnDwYnFkb3RqdVTQRcuhGf4M*';
      const options: GenerateKeyOptions = Iron.clone(Iron.defaults).integrity;
      options.salt = 'ff2bf988aa0edf2b34c02d220a45c4a3c572dac6b995771ed20de58da919bfa5';
    ...
    test('returns an error when decrypted object is invalid', async () => {
    ...
    test('returns an error when expired', async () => {
      const macBaseString =
    ...
    test('returns an error when expiration NaN', async () => {
      const macBaseString =
