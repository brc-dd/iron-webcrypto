...
describe('Iron', () => {
  ...
  it('fails to turns object into a ticket (password buffer too short)', async () => {
    ...
    await expect(Iron.seal(obj, key, Iron.defaults)).to.be.rejectedWith(
  ...
  it('fails to turn object into a ticket (failed to stringify object)', async () => {
    ...
    cyclic[0] = cyclic;
    const key = Iron.randomBits(128);
    await expect(Iron.seal(cyclic, key, Iron.defaults)).to.be.rejectedWith(
  ...
  it('handles separate password buffers (password object)', async () => {
    const key = { id: '1', encryption: Iron.randomBits(256), integrity: Iron.randomBits(256) };
    const sealed = await Iron.seal(obj, key, Iron.defaults);
  ...
  it('fails to parse a sealed object when password not found', async () => {
    const sealed = await Iron.seal(obj, { id: '1', secret: password }, Iron.defaults);
    await expect(Iron.unseal(sealed, { '2': password }, Iron.defaults)).to.be.rejectedWith(
  ...
  describe('generateKey()', () => {
    it('returns an error when password is missing', async () => {
      // @ts-ignore
      await expect(Iron.generateKey(null, null)).to.be.rejectedWith('Empty password');
    });

    it('returns an error when password is too short', async () => {
      await expect(Iron.generateKey('password', Iron.defaults.encryption)).to.be.rejectedWith(
    ...
    it('returns an error when options are missing', async () => {
      // @ts-ignore
      await expect(Iron.generateKey(password, null)).to.be.rejectedWith('Bad options');
      // @ts-ignore
      await expect(Iron.generateKey(password, 'abc')).to.be.rejectedWith('Bad options');
    });

    it('returns an error when an unknown algorithm is specified', async () => {
      // @ts-ignore
      await expect(Iron.generateKey(password, { algorithm: 'unknown' })).to.be.rejectedWith(
    ...
    it('returns an error when no salt and no salt bits are provided', async () => {
      ...
      await expect(Iron.generateKey(password, options)).to.be.rejectedWith(
  ...
  describe('encrypt()', () => {
    it('returns an error when password is missing', async () => {
      // @ts-ignore
      await expect(Iron.encrypt(null, null, 'data')).to.be.rejectedWith('Empty password');
    });
  });
  ...
  describe('decrypt', () => {
    it('returns an error when password is missing', async () => {
      // @ts-ignore
      await expect(Iron.decrypt(null, null, 'data')).to.be.rejectedWith('Empty password');
    });
  });
  ...
  describe('hmacWithPassword()', () => {
    it('returns an error when password is missing', async () => {
      // @ts-ignore
      await expect(Iron.hmacWithPassword(null, null, 'data')).to.be.rejectedWith('Empty password');
    });
  ...
  describe('seal()', () => {
    it('returns an error when password is missing', async () => {
      // @ts-ignore
      await expect(Iron.seal('data', null, Iron.defaults)).to.be.rejectedWith('Empty password');
    });
    ...
    it('returns an error when integrity options are missing', async () => {
      ...
      await expect(Iron.seal('data', password, options)).to.be.rejectedWith(
  ...
  describe('unseal()', () => {
    ...
    it('returns an error when number of sealed components is wrong', async () => {
      ...
      await expect(Iron.unseal(ticket, password, Iron.defaults)).to.be.rejectedWith(
    ...
    it('returns an error when password is missing', async () => {
      ...
      // @ts-ignore
      await expect(Iron.unseal(ticket, null, Iron.defaults)).to.be.rejectedWith('Empty password');
    });
    ...
    it('returns an error when decryption fails', async () => {
      ...
      await expect(Iron.unseal(ticket, password, Iron.defaults)).to.be.rejectedWith('bad decrypt');
    });
    ...
    it('returns an error when expired', async () => {
      ...
      const mac = await Iron.hmacWithPassword(password, options, macBaseString);
      const ticket = `${macBaseString}*${mac.salt}*${mac.digest}`;
      await expect(Iron.unseal(ticket, password, Iron.defaults)).to.be.rejectedWith('Expired seal');
    });
